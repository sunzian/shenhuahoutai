        ...
        <div class="container" v-if="!showSell">
            ...
            >
                ...
                <el-table-column prop="sort" label="套餐类型" width="150">
                    <template slot-scope="scope">
                        <el-tag v-if="scope.row.comboType=='1'"
                        >单人套餐</el-tag>
                        <el-tag v-else-if="scope.row.comboType=='2'"
                        >双人套餐</el-tag>
                        <el-tag v-else-if="scope.row.comboType=='3'"
                        >多人套餐</el-tag>
                    </template>
                </el-table-column>
                <el-table-column label="操作" width="100" align="center" fixed="right">
        ...
        methods: {
            ...
            delChange(index, row){//删除数据
                ...
                setTimeout(() => {
                    ...
                    jsonArr.push({key:"sign",value:sign});
                    let params = ParamsAppend(jsonArr);
                    https.fetchPost('/merchandiseCombo/deleteMerchandiseCombo',params).then((data) => {
                        // console.log(data);
            ...
            addChange(index, row){//是否修改权限
                ...
                setTimeout(() => {
                    ...
                    jsonArr.push({key:"sign",value:sign});
                    let params = ParamsAppend(jsonArr);
                    https.fetchPost('/merchandiseCombo/modifyPage',params).then((data) => {
                        console.log(data);
                        console.log(JSON.parse(Decrypt(data.data.data)));
                        if(data.data.code=='success'){
            ...
            exChanger() {
                ...
                setTimeout(() => {
                    ...
                    https.fetchPost('/merchandiseCombo/modifyMerchandiseCombo',params).then((data) => {
                        console.log(data);
                        // console.log(JSON.parse(Decrypt(data.data.data)));
                        if(data.data.code=='success'){
                            this.selectedSell=[];
            ...
            refresh(){//刷新卖品页面
                ...
                setTimeout(() => {
                    ...
                    jsonArr.push({key:"sign",value:sign});
                    var params = ParamsAppend(jsonArr);
                    https.fetchPost('/merchandiseCombo/list',params).then((data) => {
                        console.log(data);
                        if(data.data.code=='success') {
                            var oData = JSON.parse(Decrypt(data.data.data));
                            console.log(oData);

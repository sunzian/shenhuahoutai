    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 查询卖品订单
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            <div class="handle-box">
                <el-button style="float: right" type="warning" @click="addPage">查询卖品订单</el-button>
            </div>
            ...
            <el-form v-model="oForm">
                ...
                <el-form-item label="取餐状态" :label-width="formLabelWidth">
                    <el-input :disabled="true" onkeyup="this.value=this.value.replace(/\D/g,'')" style="width: 250px"
                              v-model="oForm.deliveryStatus" autocomplete="off"></el-input>
                </el-form-item>
    ...
    export default {
        ...
        methods: {
            addPage() {
                ...
                setTimeout(() => {
                    https.fetchPost('/interface/queryMerchandiseOrder', params)
                        .then(data => {
                                loading.close();
                                if (data.data.code == 'success') {
                                    ...
                                    if(JSON.parse(Decrypt(data.data.data)).deliveryStatus=="Yes"){
                                        this.oForm.deliveryStatus='已取'
                                    }else{
                                        this.oForm.deliveryStatus='未取'
                                    }
                                    if(JSON.parse(Decrypt(data.data.data)).cancelStatus=="Yes"){
                                        this.oForm.cancelStatus='已退'
                                    }else{
                                        this.oForm.cancelStatus='未退'
                                    }

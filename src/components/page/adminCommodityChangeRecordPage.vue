        ...
        derive() {
            ...
            setTimeout(() => {
                ...
                jsonArr.push({
                    key: 'exportKeysJson',
                    value:
                        "['id','cinemaCode','cinemaName','orderNo','mobile','commodityName','gold','money','payTime','chPayStatus','tradeNo','payReturnMsg','chChangeType','chStatus','chRefundStatus','refundNo','refundReason','refundApply','refundTime','refundPrice']"
                });
                jsonArr.push({
                    key: 'exportTitlesJson',
                    value:
                        "['ID','影院编码','影院名称','订单号','手机号','商品名称','消费金币','支付金额','兑换时间','兑换状态','支付交易号','支付回调消息','兑换方式','核销状态','退款状态','退款交易号','退款原因','微信退款回复','退款时间','退款金额']"
                });

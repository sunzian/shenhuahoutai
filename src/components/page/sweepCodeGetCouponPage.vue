    ...
    <div>
        ...
        <el-dialog :close-on-click-modal="false" title="新增" :visible.sync="dialogFormVisible">
            <el-form :model="oForm">
                ...
                <span style="margin-left: 170px;font-size: 12px;color: red;">说明：是，活动期间每天可领取1次；否，活动期间只能领取1次。</span>
        ...
        <el-dialog :close-on-click-modal="false" title="修改" :visible.sync="editVisible">
            <el-form :model="form">
                ...
                <span
                    style="margin-left: 170px;font-size: 12px;color: red;"
                >说明：是，活动期间每天可领取1次；否，活动期间只能领取1次。</span>
                <!--<el-form-item :required="true" label="用户限购次数" :label-width="formLabelWidth">-->
    ...
    methods: {
        ...
        openNext() {
            ...
            setTimeout(() => {
                ...
                https
                    .fetchPost('/merchandiseCoupon/getCouponByCinemaCodeForGroup', params)
                    .then(data => {
                        loading.close();
        ...
        addRole() {
            ...
            if (!this.oForm.name || this.oForm.name.trim() == '') {
                this.message = '请填写活动名称！';
                this.open();
                return;
            }
            if (!this.oForm.startDate || this.oForm.startDate == '') {
                this.message = '请选择活动开始时间！';
                this.open();
                return;
            }
            if (!this.oForm.endDate || this.oForm.endDate == '') {
            ...
            if (this.dialogFormVisible == true) {
                https
                    .fetchPost('/sweepCodeGetCoupon/add', params)
                    .then(data => {
                        loading.close();
                        console.log(data);
                        if (data.data.code == 'success') {
        ...
        // 编辑操作
        exChanger() {
            if (this.oCommonType == 2 && this.oCinemaCodes.length == 0) {
                this.message = '请选择影院！';
                this.open();
                return;
            }
            if (!this.oName || this.oName.trim() == '') {
                this.message = '请填写活动名称！';
                this.open();
                return;
            }
            if (!this.oStartDate || this.oStartDate == '') {
                this.message = '请选择活动开始时间！';
                this.open();
                return;
            }
            if (!this.oEndDate || this.oEndDate == '') {
                this.message = '请选择活动结束时间！';
                this.open();
                return;
            }

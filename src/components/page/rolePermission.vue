    ...
    <div>
        ...
        <el-dialog :close-on-click-modal="false" title="编辑" :visible.sync="editVisible" width="30%">
            <el-form ref="form" :model="form" label-width="100px">
                <el-checkbox v-model="coverStatus">是否覆盖该角色下的所有账号</el-checkbox>
                <el-form-item label="权限">
                    <el-tree
                        ref="tree_"
                        :data="data"
    ...
    methods: {
        viewRolePermission(index, row) {
            ...
            setTimeout(() => {
                ...
                https
                    .fetchPost('/role/viewRolePermission', params)
                    .then(data => {
                        loading.close();
        ...
        addChange(index, row) {
            ...
            setTimeout(() => {
                ...
                let params = ParamsAppend(jsonArr);
                https
                    .fetchPost('/role/permissionForRolePage', params)
                    .then(data => {
                        loading.close();
                        if (data.data.code == 'success') {
        ...
        // 编辑操作
        exChanger() {
            if (this.coverStatus == true) {
                this.$confirm('将覆盖该角色下的所有账号权限，是否继续', '提示', {
                    ...
                    .then(() => {
                        ...
                        setTimeout(() => {
                            var jsonArr = [];
                            jsonArr.push({ key: 'coverStatus', value: 2 });
                            ...
                            let params = ParamsAppend(jsonArr);
                            https
                                .fetchPost('/role/permissionForRole', params)
                                .then(data => {
                                    loading.close();
                                    if (data.data.code == 'success') {
                ...
                setTimeout(() => {
                    var jsonArr = [];
                    jsonArr.push({ key: 'coverStatus', value: 1 });
                    jsonArr.push({ key: 'id', value: this.form.id });
                    jsonArr.push({
                        key: 'permissionIds',
                        value: this.$refs.tree_.getCheckedKeys().concat(this.$refs.tree_.getHalfCheckedKeys())
        ...
        getMenu() {
            ...
            setTimeout(() => {
                ...
                https
                    .fetchPost('/role/rolePermission', params)
                    .then(data => {
                        loading.close();
                        if (data.data.code == 'success') {

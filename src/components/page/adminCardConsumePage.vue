    ...
    methods: {
        derive(){
            ...
            setTimeout(() => {
                ...
                jsonArr.push({ key: 'exportKeysJson', value: "['cinemaCode','cinemaName','consumeCinemaCode','consumeCinemaName','cardNo','userName','mobilePhone','consumeAmount','consumeDetail','chStatus','orderCode','chOrderType','consumeTime','errorMessage','userName','orderCode']"});
                jsonArr.push({ key: 'exportTitlesJson', value:"['开卡影院编码','开卡影院名称','消费影院编码','消费影院名称','消费卡号','会员卡所属用户','消费手机号','消费金额','消费明细','支付状态','订单号','订单类型','消费时间','接口调用结果','会员卡所属用户','订单号']" });

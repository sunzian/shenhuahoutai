    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item> <i class="el-icon-lx-cascades"></i> 金币商品设置 </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
            <div class="handle-box">
                ...
                <el-select clearable v-model="query.commodityCategory" placeholder="商品分类" class="handle-select mr10">
                ...
                <el-select clearable v-model="query.recommendStatus" placeholder="是否推荐" class="handle-select mr10">
                    <el-option key="1" label="否" value="1"></el-option>
                    <el-option key="2" label="是" value="2"></el-option>
                </el-select>
                <el-button style="margin-top: 10px;width: 90px;" type="primary" icon="el-icon-search" @click="Search">搜索</el-button>
                <el-button type="primary" @click="addPage" icon="el-icon-circle-plus-outline" style="float: right;margin-top: 10px"
                    >新增</el-button
                >
            </div>
            <el-table
            ...
            >
                ...
                <el-table-column prop="changeNumber" label="已兑换数量" width="120" sortable="custom">
                ...
                <el-table-column prop="sort" label="排序" width="160">
                    <template slot-scope="scope">
                        ...
                        <el-input
                            ...
                            @keyup.enter.native="changeScope(scope.$index, scope.row)"
                            @change="handleEdit(scope.$index, scope.row)"
                        ></el-input>
                        <el-button
                            v-if="scope.row.showEdit"
                            style="position: absolute;left: 93px;"
                            @click="changeScope(scope.$index, scope.row)"
                            >确定</el-button
                        >
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作" width="140" align="center" fixed="right">
                    <template slot-scope="scope">
                        <el-button type="text" icon="el-icon-edit" @click="addChange(scope.$index, scope.row)">编辑</el-button>
                        <el-button type="text" icon="el-icon-edit" @click="copyActivity(scope.$index, scope.row)">复制</el-button>
                        <el-button type="text" icon="el-icon-delete" class="red" @click="delChange(scope.$index, scope.row)"
                            >删除</el-button
                        >
                    </template>
                </el-table-column>
            ...
            <el-form v-model="oForm">
                <el-form-item :required="true" label="允许售卖的门店" :label-width="formLabelWidth">
                    ...
                    <el-checkbox-group v-model="oCheckedCities" @change="getCinemaCode">
                        <el-checkbox v-for="city in cities" :label="city.cinemaCode" :key="city.cinemaCode" :value="city.cinemaCode">{{
                            city.cinemaName
                        }}</el-checkbox>
                    </el-checkbox-group>
                </el-form-item>
                <el-form-item :required="true" label="商品类型" :label-width="formLabelWidth">
                    <el-select v-model="oForm.commodity_type" placeholder="请选择商品类型" @change="changeAddType">
                        <el-option v-for="item in commodityType" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                ...
                <el-form-item v-if="oForm.commodity_type == 4" :required="true" label="选择商户" :label-width="formLabelWidth">
                    <el-button size="small" type="primary" @click="getPartner()">点击选择</el-button>
                </el-form-item>
                <el-form-item
                    v-if="selectedPartner.length > 0 && oForm.commodity_type == 4"
                    label="所选商户："
                    :label-width="formLabelWidth"
                    :required="true"
                >
                    <div v-for="(item, index) in selectedPartner" style="margin-bottom: 5px" :key="index">
                        <el-input
                            style="width: 280px"
                        ...
                        ></el-input
                        >&nbsp;&nbsp;&nbsp;&nbsp;
                        <span style="color:red;cursor: pointer;" @click="deletPartner(index)">删除</span>
                    </div>
                </el-form-item>
                <el-form-item
                ...
                >
                    <el-select v-model="oForm.supportExpressStatus" placeholder="请选择取货方式">
                        <el-option v-for="item in supportType" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item
                ...
                <el-form-item :required="true" label="商品图片" :label-width="formLabelWidth">
                    <el-popover placement="right" title trigger="hover">
                        <img :src="oForm.image_url" />
                        <img slot="reference" :src="oForm.image_url" :alt="oForm.image_url" style="max-height: 50px;max-width: 130px" />
                    </el-popover>
                    <el-upload
                        class="upload-demo"
                    ...
                    >
                        ...
                        <div slot="tip" class="el-upload__tip">只能上传jpg/png文件，且不超过200kb 建议尺寸200*200或按比例上传</div>
                    </el-upload>
                </el-form-item>
                <el-form-item
                    :required="true"
                ...
                <el-form-item :required="true" label="兑换方式" :label-width="formLabelWidth" v-if="oForm.commodity_type != 5">
                    <el-select v-model="oForm.change_type" placeholder="请选择兑换方式" @change="clearList()">
                        <el-option v-for="item in showType" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                ...
                <el-form-item
                    v-if="oForm.isUseSku == 1 && (oForm.commodity_type == 1 || oForm.commodity_type == 4)"
                    :required="true"
                    label="商品规格属性"
                    :label-width="formLabelWidth"
                >
                    <el-button type="primary" @click="addUseSku" icon="el-icon-circle-plus-outline">新增</el-button>
                    <el-table
                        :data="skuList"
                        border
                        class="table"
                ...
                >
                    <el-select v-model="oForm.effectiveType" placeholder="请选择生效方式">
                        <el-option v-for="item in effectiveType" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item
                    :required="true"
                ...
                <el-form-item
                    ...
                    label="优惠券有效期："
                    :label-width="formLabelWidth"
                    v-if="oForm.effectiveType == 2 && oForm.commodity_type != 1 && oForm.commodity_type != 4 && oForm.commodity_type != 5"
                >
                    <el-date-picker
                ...
                >
                    <el-date-picker
                        ...
                        format="yyyy-MM-dd"
                    ></el-date-picker>
                    <div style="color: red;">如不填写，则表示不限制</div>
                </el-form-item>
                <el-form-item
                    label="商品截止核销时间："
                    :label-width="formLabelWidth"
                ...
                <el-form-item :required="true" label="是否今日大牌" :label-width="formLabelWidth">
                    <el-select v-model="oForm.topStatus" placeholder="请选择">
                        <el-option v-for="item in topStatusList" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                ...
                <el-form-item :required="true" label="上架状态" :label-width="formLabelWidth">
                    <el-select v-model="oForm.status" placeholder="请选择上架状态">
                        <el-option v-for="item in showStatus" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="下架日期：" :label-width="formLabelWidth">
                    <el-date-picker
                ...
                <el-form-item label="商品基础销量值" :label-width="formLabelWidth">
                    ...
                    ></el-input>
                    <div style="color: red;">小程序展示销量=基础销量+实际销量</div>
                </el-form-item>
                ...
                <el-form-item :required="true" v-if="oForm.commodity_type == 3" label="选择券包" :label-width="formLabelWidth">
                    <el-button type="primary" @click="changeCoupon">点击选择</el-button>
                </el-form-item>
                <el-form-item
                    v-if="oForm.commodity_type == 3 && couponId"
                    label="所选券包："
                    :label-width="formLabelWidth"
                    :required="true"
                >
                    <el-input style="width: 150px" v-model="groupName" autocomplete="off" disabled></el-input>&nbsp;&nbsp;&nbsp;&nbsp;
                    <span v-if="groupName" style="color:red;cursor: pointer;" @click="deletCoupon">删除</span>
                </el-form-item>
                <el-form-item v-if="oForm.commodity_type == 1" label="选择券包" :label-width="formLabelWidth">
                    <el-button type="primary" @click="changeCoupon">点击选择</el-button>
                </el-form-item>
                <el-form-item v-if="oForm.commodity_type == 1 && couponId" label="所选券包：" :label-width="formLabelWidth">
                    <el-input style="width: 150px" v-model="groupName" autocomplete="off" disabled></el-input>&nbsp;&nbsp;&nbsp;&nbsp;
                    <span v-if="groupName" style="color:red;cursor: pointer;" @click="deletCoupon">删除</span>
                </el-form-item>
                <el-form-item
                ...
                >
                    <el-input
                        style="width: 250px"
                        placeholder="如仅限当天使用，填0"
                        v-model.trim="oForm.couponExpireDay"
                        autocomplete="off"
                        onkeyup="this.value=this.value.replace(/\D/g,'')"
                    ></el-input>
                </el-form-item>
                ...
                <el-form-item :required="true" v-if="oForm.commodity_type == 5" label="选择权益卡" :label-width="formLabelWidth">
                ...
                >
                    ...
                    <span v-if="cardName" style="color:red;cursor: pointer;" @click="deletCard">删除</span>
                </el-form-item>
                ...
                <el-form-item :required="true" label="是否限购" :label-width="formLabelWidth">
                    <el-select v-model="oForm.limit_type" placeholder="请选择限制时间">
                        <el-option v-for="item in limitType" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item
                ...
                <el-form-item :required="true" label="是否限制每日售卖数量" :label-width="formLabelWidth">
                    <el-select v-model="oForm.dayLimitStatus" placeholder="请选择限制时间">
                        <el-option v-for="item in dayLimitStatus" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item v-if="oForm.dayLimitStatus == 2" label="每日可售卖数量" :label-width="formLabelWidth" :required="true">
                    <el-input
                        onkeyup="this.value=this.value.replace(/\D/g,'')"
                        style="width: 200px"
                        v-model="oForm.dayLimitNumber"
                        autocomplete="off"
                ...
                <el-form-item :required="true" v-if="oForm.change_type != 1" label="分销提成方式" :label-width="formLabelWidth">
                    <el-select v-model="oForm.commissionWay" @change="clearCommission" placeholder="请选择分销提成方式">
                        <el-option
                            v-for="item in commissionWayType"
                            :key="item.value"
                            :label="item.label"
                            :value="item.value"
                            :disabled="item.disabled"
                ...
                <el-form-item
                    :required="true"
                    v-if="oForm.change_type != 1 && oForm.commissionWay == '1'"
                    label="一级佣金金额"
                    :label-width="formLabelWidth"
                >
                    <el-input
                        onkeyup="this.value=this.value.replace(/[^0-9.]+/,'')"
                        style="width: 250px"
                        v-model.trim="oForm.firstCommission"
                        autocomplete="off"
                        placeholder="请输入正确的数字"
                    ></el-input>
                </el-form-item>
                <el-form-item
                    :required="true"
                    v-if="oForm.change_type != 1 && oForm.commissionWay == '2'"
                    label="一级佣金百分比"
                    :label-width="formLabelWidth"
                >
                    <el-input
                    ...
                    ></el-input>
                    <br>
                    <span style="color: red;font-size: 12px;">（按用户实付金额计算，输入数字为0.01-1，例：提成为百分之一，填入0.01）</span>
                </el-form-item>
                <el-form-item
                    :required="true"
                    v-if="oForm.change_type != 1 && oForm.commissionWay == '1'"
                ...
                >
                    <el-input
                        onkeyup="this.value=this.value.replace(/[^0-9.]+/,'')"
                        style="width: 250px"
                        v-model.trim="oForm.secondCommission"
                        autocomplete="off"
                        placeholder="请输入正确的数字"
                    ></el-input>
                </el-form-item>
            ...
            <el-form ref="form" :model="form">
                <el-form-item :required="true" label="允许售卖的门店" :label-width="formLabelWidth">
                    ...
                    <el-checkbox-group v-model="oCheckedCities" @change="changeCinema" v-if="form.commodityCategory != 5">
                        <el-checkbox v-for="city in oCities" :label="city.cinemaCode" :key="city.cinemaCode" :value="city.cinemaCode">{{
                            city.cinemaName
                        }}</el-checkbox>
                    </el-checkbox-group>
                ...
                <el-form-item v-if="form.commodityType == 4" :required="true" label="选择商户" :label-width="formLabelWidth">
                    <el-button size="small" type="primary" @click="getPartner()">点击选择</el-button>
                </el-form-item>
                <el-form-item
                    v-if="selectedPartner.length > 0 && form.commodityType == 4"
                    label="所选商户："
                    :label-width="formLabelWidth"
                    :required="true"
                >
                ...
                <el-form-item
                    v-if="form.commodityType == 4 || form.commodityType == 1"
                    :required="true"
                    label="取货方式"
                    :label-width="formLabelWidth"
                >
                ...
                <el-form-item :required="true" label="商品图片" :label-width="formLabelWidth">
                    <el-popover placement="right" title trigger="hover">
                        <img :src="oImageUrl" />
                        <img slot="reference" :src="oImageUrl" :alt="oImageUrl" style="max-height: 50px;max-width: 130px" />
                    </el-popover>
                    <el-upload
                        :before-upload="beforeUpload"
                        :data="type"
                ...
                <el-form-item v-if="form.isUseSku == 1" :required="true" label="商品规格属性" :label-width="formLabelWidth">
                    <el-button type="primary" @click="addChangeUseSku" icon="el-icon-circle-plus-outline">新增</el-button>
                    <el-table
                        :data="changeSkuList"
                        border
                        class="table"
                        ...
                        <el-table-column label="操作" width="100" align="center" fixed="right">
                            <template slot-scope="scope">
                                ...
                                <el-button
                                    ...
                                    icon="el-icon-delete"
                                    class="red"
                                    @click="delChangeSku(scope.$index, scope.row)"
                                ></el-button>
                            </template>
                        </el-table-column>
                    </el-table>
                ...
                <el-form-item :required="true" v-if="form.commodityType == 3" label="选择券包" :label-width="formLabelWidth">
                    <el-button type="primary" @click="changeCoupon">点击选择</el-button>
                </el-form-item>
                <el-form-item v-if="couponId && form.commodityType == 3" label="所选券包：" :label-width="formLabelWidth" :required="true">
                    <el-input style="width: 150px" v-model="groupName" autocomplete="off" disabled></el-input>&nbsp;&nbsp;&nbsp;&nbsp;
                    <span v-if="groupName" style="color:red;cursor: pointer;" @click="deletCoupon">删除</span>
                </el-form-item>
                <el-form-item v-if="form.commodityType == 1" label="选择券包" :label-width="formLabelWidth">
                    <el-button type="primary" @click="changeCoupon">点击选择</el-button>
                </el-form-item>
                <el-form-item v-if="form.commodityType == 1 && couponId" label="所选券包：" :label-width="formLabelWidth">
                    <el-input style="width: 150px" v-model="groupName" autocomplete="off" disabled></el-input>&nbsp;&nbsp;&nbsp;&nbsp;
                    <span v-if="groupName" style="color:red;cursor: pointer;" @click="deletCoupon">删除</span>
                </el-form-item>
                <el-form-item
                ...
                >
                    <el-input
                        style="width: 250px"
                        placeholder="如仅限当天使用，填0"
                        v-model.trim="form.couponExpireDay"
                        autocomplete="off"
                        onkeyup="this.value=this.value.replace(/\D/g,'')"
                    ></el-input>
                </el-form-item>
                ...
                <el-form-item :required="true" v-if="form.commodityType == 5" label="选择权益卡" :label-width="formLabelWidth">
                    <el-button type="primary" @click="changeCard">点击选择</el-button>
                </el-form-item>
                <el-form-item v-if="cardId && form.commodityType == 5" label="所选权益卡：" :label-width="formLabelWidth" :required="true">
                    <el-input style="width: 150px" v-model="cardName" autocomplete="off" disabled></el-input>&nbsp;&nbsp;&nbsp;&nbsp;
                    <span v-if="cardName" style="color:red;cursor: pointer;" @click="deletCard">删除</span>
                </el-form-item>
                <el-form-item :required="true" label="是否指定日期可以兑换" :label-width="formLabelWidth">
                ...
                <el-form-item v-if="form.assignType == 2" label="选择指定日期" :label-width="formLabelWidth" :required="true">
                    <el-date-picker
                ...
                <el-form-item v-if="form.assignType == 3" label="输入每月几号" :label-width="formLabelWidth" :required="true">
                    <el-select v-model="form.assignInfo">
                        <el-option v-for="item in days" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item v-if="form.assignType == 4" label="输入每月第几周" :label-width="formLabelWidth" :required="true">
                    <el-select v-model="form.assignInfo">
                        <el-option v-for="item in weeks" :key="item.value" :label="item.label" :value="item.value"></el-option>
                    </el-select>
                </el-form-item>
                ...
                <el-form-item v-if="form.dayLimitStatus == 2" label="每日可售卖数量" :label-width="formLabelWidth" :required="true">
                    <el-input
                        onkeyup="this.value=this.value.replace(/\D/g,'')"
                        style="width: 200px"
                ...
                <el-form-item :required="true" v-if="form.changeType != 1" label="分销提成方式" :label-width="formLabelWidth">
                    <el-select v-model="oCommissionWay" @change="clearCommission" placeholder="请选择分销提成方式">
                        <el-option
                            v-for="item in commissionWayType"
                            :key="item.value"
                            :label="item.label"
                ...
                <el-form-item
                    :required="true"
                    v-if="form.changeType != 1 && oCommissionWay == '1'"
                    label="一级佣金金额"
                    :label-width="formLabelWidth"
                >
                    <el-input
                ...
                <el-form-item
                    :required="true"
                    v-if="form.changeType != 1 && oCommissionWay == '2'"
                    label="一级佣金百分比"
                    :label-width="formLabelWidth"
                >
                    <el-input
                ...
                >
                    <el-input
                        onkeyup="this.value=this.value.replace(/[^0-9.]+/,'')"
                        style="width: 250px"
                        v-model.trim="oSecondCommission"
                        autocomplete="off"
                        placeholder="请输入正确的数字"
                    ></el-input>
                </el-form-item>
            ...
            <div class="container">
                ...
                >
                    ...
                    <el-table-column label="优惠券详情">
                        <template slot-scope="scope">
                            <span v-for="(item, index) in scope.row.couponList" :key="index"
        ...
        <el-dialog :close-on-click-modal="false" title="选择权益卡" :visible.sync="drawerCard">
            <div class="container">
                <div class="handle-box">
                    <el-input v-model="query.cardName" placeholder="权益卡名称" class="handle-input mr12"></el-input>
                ...
                >
                    ...
                    <el-table-column label="权益卡有效期" width="150">
                        <template slot-scope="scope"
                            >{{ scope.row.number }}{{ scope.row.unit }}</template
                        >
                    </el-table-column>
        ...
        <el-dialog :close-on-click-modal="false" title="选择商户" :visible.sync="drawerPartner">
            <div class="container">
                <div class="handle-box">
                    ...
                    ></el-input>
                    <el-input v-model="query.name" placeholder="合作商名称" class="handle-input mr12"></el-input>
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="surePartner">确 定</el-button>
            </div>
        </el-dialog>
        <!--新增sku弹出框新增-->
        <el-dialog :close-on-click-modal="false" title="新增" :visible.sync="dialogskuFormVisible">
            <el-form :model="skuForm">
                <el-form-item :required="true" label="规格" :label-width="formLabelWidth">
                    <el-input
                        placeholder="例如：颜色、包装..."
                        style="width: 350px"
                        min="1"
                        maxlength="30"
                        v-model.trim="skuForm.specifications"
                        autocomplete="off"
                    ></el-input>
                </el-form-item>
                <el-form-item label="属性" :label-width="formLabelWidth">
                    <el-input
                        placeholder="例如：尺寸、容量..."
                        style="width: 200px"
                        min="1"
                        maxlength="10"
                        v-model="skuForm.attribute"
                        autocomplete="off"
                    ></el-input>
                </el-form-item>
                <el-form-item label="图片" :label-width="formLabelWidth">
                    <el-popover placement="right" title trigger="hover">
                        ...
                        <img slot="reference" :src="skuForm.image" :alt="skuForm.image" style="max-height: 50px;max-width: 130px" />
                    </el-popover>
                    <span v-if="skuForm.image" style="color:red;cursor: pointer;margin-left:20px;" @click="deletImg">删除</span>
                    <el-upload
                        :before-upload="beforeUpload"
                        :data="type"
                        :limit="1"
            ...
            <el-form :model="oSkuForm">
                <el-form-item :required="true" label="规格" :label-width="formLabelWidth">
                    <el-input
                        ...
                        min="1"
                        maxlength="30"
                        v-model.trim="oSkuForm.specifications"
                        autocomplete="off"
                    ></el-input>
                </el-form-item>
                <el-form-item label="属性" :label-width="formLabelWidth">
                ...
                <el-form-item label="图片" :label-width="formLabelWidth">
                    <el-popover placement="right" title trigger="hover">
                        ...
                        <img slot="reference" :src="oSkuForm.image" :alt="oSkuForm.image" style="max-height: 50px;max-width: 130px" />
                    </el-popover>
                    <span v-if="oSkuForm.image" style="color:red;cursor: pointer;margin-left:20px;" @click="deletImg2">删除</span>
                    <el-upload
                        :before-upload="beforeUpload"
                        :data="type"
                        :limit="1"
            ...
            <el-form :model="skuChangeForm">
                <el-form-item :required="true" label="规格" :label-width="formLabelWidth">
                    <el-input
                        ...
                        min="1"
                        maxlength="30"
                        v-model.trim="skuChangeForm.specifications"
                        autocomplete="off"
                    ></el-input>
                </el-form-item>
                <el-form-item label="属性" :label-width="formLabelWidth">
                    <el-input
                        ...
                        min="1"
                        maxlength="10"
                        v-model="skuChangeForm.attribute"
                        autocomplete="off"
                    ></el-input>
                </el-form-item>
                <el-form-item label="图片" :label-width="formLabelWidth">
                    ...
                    <span v-if="skuChangeForm.image" style="color:red;cursor: pointer;margin-left:20px;" @click="deletImg3">删除</span>
                    <el-upload
                        :before-upload="beforeUpload"
                        :data="type"
                        :limit="1"
            ...
            <el-form :model="oSkuChangeForm">
                <el-form-item :required="true" label="规格" :label-width="formLabelWidth">
                    <el-input
                        ...
                        min="1"
                        maxlength="30"
                        v-model.trim="oSkuChangeForm.specifications"
                        autocomplete="off"
                    ></el-input>
                </el-form-item>
                <el-form-item label="属性" :label-width="formLabelWidth">
                    <el-input
                        ...
                        min="1"
                        maxlength="10"
                        v-model="oSkuChangeForm.attribute"
                        autocomplete="off"
                    ></el-input>
                </el-form-item>
                <el-form-item label="图片" :label-width="formLabelWidth">
                    ...
                    <span v-if="oSkuChangeForm.image" style="color:red;cursor: pointer;margin-left:20px;" @click="deletImg4">删除</span>
                    <el-upload
                        :before-upload="beforeUpload"
                        :data="type"
                        :limit="1"
    ...
    methods: {
        sortChange(column, prop, order) {
            if (column.order == 'ascending') {
                this.query.changeSort = '2';
            } else if (column.order == 'descending') {
                this.query.changeSort = '1';
            } else if (!column.order) {
                this.query.changeSort = '';
            }
            this.getMenu();
        },
        ...
        surePartner() {
            ...
            let selectedPartner = [];
            this.selectedPartner = selectedPartner.concat(this.multipleSelection);
            this.drawerPartner = false;
        },
        deletPartner(index) {
            this.selectedPartner.splice(index, 1);
        },
        changePartnerName(e) {
            this.$forceUpdate();
        },
        ...
        getPartner() {
            ...
            jsonArr.push({ key: 'brandBusinessName', value: name });
        ...
        clearCommission() {
            this.oForm.firstCommission = '';
            this.oForm.secondCommission = '';
            this.oFirstCommission = '';
            this.oSecondCommission = '';
        },
        deletCoupon() {
            this.groupName = '';
            this.couponId = '';
        ...
        addRole() {
            ...
            // 如果选择了实物又选择了券包
            if (this.oForm.commodity_type == 1 && this.couponId && this.couponId != '') {
                ...
                if (this.oForm.effectiveType == 1) {
                    ...
                    if (this.oForm.couponExpireDay === '') {
                        this.message = '券包有效期不能为空，请检查！';
                        this.open();
                        loading.close();
                        return;
                    }
                    if (this.oForm.couponExpireDay < 0) {
                        this.message = '券包有效期不能小于0，请检查！';
                        this.open();
                        loading.close();
                        return;
                    }
            ...
            if (
                ...
                this.oForm.commodity_type != 1 &&
                this.oForm.commodity_type != 4 &&
                this.oForm.commodity_type != 5
            ) {
                if (this.oForm.laterDays === '') {
                    this.message = '领取后几天开始生效不能为空，请检查！';
                    this.open();
            ...
            if (!this.oForm.dayLimitStatus || this.oForm.dayLimitStatus == '') {
                this.message = '限制每日售卖数量不能为空，请检查！';
                this.open();
                loading.close();
                return;
            }
            if (this.oForm.dayLimitStatus == 2) {
                if (!this.oForm.dayLimitNumber || this.oForm.dayLimitNumber <= 0) {
                    this.message = '每日售卖数量必须大于0，请检查！';
                    this.open();
                    loading.close();
                    return;
                }
            ...
            if (this.oForm.commissionWay == 1 || this.oForm.commissionWay == 2) {
                if (
                    this.oForm.firstCommission == '' ||
                    this.oForm.firstCommission <= 0 ||
                    this.oForm.secondCommission == '' ||
                    this.oForm.secondCommission <= 0
                ) {
                    this.message = '分销提成金额必须大于0！';
                    this.open();
                    loading.close();
                    return;
                }
            ...
            jsonArr.push({ key: 'verifyDate', value: this.oForm.verifyDate });
            jsonArr.push({ key: 'endVerifyDate', value: this.oForm.endVerifyDate });
            jsonArr.push({ key: 'baseSaleNumber', value: this.oForm.baseSaleNumber });
            jsonArr.push({ key: 'dayLimitStatus', value: this.oForm.dayLimitStatus });
            if (this.oForm.dayLimitStatus == 2) {
                jsonArr.push({ key: 'dayLimitNumber', value: this.oForm.dayLimitNumber });
            }
            ...
            if (this.oForm.commodity_type == 4) {
                ...
                for (let i = 0; i < this.selectedPartner.length; i++) {
                    partnerCode.push(this.selectedPartner[i].partnerCode);
                }
                jsonArr.push({ key: 'partnerCode', value: partnerCode.join(',') });
            ...
            if (this.oForm.commodity_type == 1) {
                ...
                if (this.couponId && this.couponId != '') {
                    ...
                    jsonArr.push({ key: 'couponName', value: this.groupName });
                    if (this.oForm.effectiveType == 1) {
                        jsonArr.push({ key: 'laterDays', value: this.oForm.laterDays });
                        jsonArr.push({ key: 'couponExpireDay', value: this.oForm.couponExpireDay });
                    } else {
            ...
            } else if (this.oForm.commodity_type != 1) {
            ...
            if (
                this.oForm.commodity_type == 2 ||
                this.oForm.commodity_type == 3 ||
                this.oForm.commodity_type == 4 ||
                this.oForm.commodity_type == 5
            ) {
                if (this.oForm.effectiveType == 1) {
            ...
            if (this.oForm.change_type != 1) {
                jsonArr.push({ key: 'commissionWay', value: this.oForm.commissionWay });
                if (this.oForm.commissionWay == 1 || this.oForm.commissionWay == 2) {
                    jsonArr.push({ key: 'firstCommission', value: this.oForm.firstCommission });
                    jsonArr.push({ key: 'secondCommission', value: this.oForm.secondCommission });
                }
            }
            let sign = md5(preSign(jsonArr));
            jsonArr.push({ key: 'sign', value: sign });
                ...
                https
                    ...
                    .then(data => {
                        ...
                        if (data.data.code == 'success') {
                            ...
                            this.partnerCode = '';
                            this.oForm.endDate = '';
                            this.oForm.verifyDate = '';
                            this.oForm.endVerifyDate = '';
                            this.oForm.couponExpireDay = '';
                            this.oForm.supportExpressStatus = '1';
                            this.oForm.dayLimitStatus = '1';
                            this.oForm.dayLimitNumber = '';
                            this.oForm.commissionWay = '-1';
                            this.oForm.firstCommission = '';
                            this.oForm.secondCommission = '';
                ...
                https
                    ...
                    .then(data => {
                        ...
                        if (data.data.code == 'success') {
                            ...
                            if (
                                JSON.parse(Decrypt(data.data.data)).goldCommodity.partnerCodeList &&
                                JSON.parse(Decrypt(data.data.data)).goldCommodity.partnerNameList &&
                                JSON.parse(Decrypt(data.data.data)).goldCommodity.partnerCodeList.length > 0 &&
                                JSON.parse(Decrypt(data.data.data)).goldCommodity.partnerNameList.length > 0
                            ) {
                                let partnerCodeList = JSON.parse(Decrypt(data.data.data)).goldCommodity.partnerCodeList.join(',');
                                let partnerNameList = JSON.parse(Decrypt(data.data.data)).goldCommodity.partnerNameList.join(',');
                                let exFilmCodeList = partnerCodeList.split(',');
                                let exFilmNameList = partnerNameList.split(',');
                                this.selectedPartner = [];
                                for (let x in exFilmNameList) {
                                    let json = {};
                                    json.partnerName = exFilmNameList[x];
                                    json.partnerCode = exFilmCodeList[x];
                                    this.selectedPartner.push(json);
                                }
                            }
                            ...
                            this.form.couponExpireDay = JSON.parse(Decrypt(data.data.data)).goldCommodity.couponExpireDay;
                            ...
                            for (let x in this.commodityCategoryType) {
                                if (
                                    this.commodityCategoryType[x].value ==
                                    JSON.parse(Decrypt(data.data.data)).goldCommodity.commodityCategory
                                ) {
                            ...
                            for (let x in this.commissionWayType) {
                                if (this.commissionWayType[x].value == JSON.parse(Decrypt(data.data.data)).goldCommodity.commissionWay) {
                                    this.oCommissionWay = this.commissionWayType[x].value;
                                    break;
                                }
                            }
                            if (this.oCommissionWay != '-1') {
                                this.oFirstCommission = JSON.parse(Decrypt(data.data.data)).goldCommodity.firstCommission;
                                this.oSecondCommission = JSON.parse(Decrypt(data.data.data)).goldCommodity.secondCommission;
                            }
        ...
        exChanger() {
            ...
            // 如果选择了实物又选择了券包
            if (this.form.commodityType == 1 && this.couponId && this.couponId != '') {
                ...
                if (this.oEffectiveType == 1) {
                    ...
                    if (this.form.couponExpireDay === '') {
                        this.message = '券包有效期不能为空，请检查！';
                        this.open();
                        loading.close();
                        return;
                    }
                    if (this.form.couponExpireDay < 0) {
                        this.message = '券包有效期不能小于0，请检查！';
                        this.open();
                        loading.close();
                        return;
            ...
            if (!this.form.dayLimitStatus || this.form.dayLimitStatus == '') {
                this.message = '限制每日售卖数量不能为空，请检查！';
                this.open();
                loading.close();
                return;
            ...
            if (this.form.dayLimitStatus == 2) {
                if (!this.form.dayLimitNumber || this.form.dayLimitNumber <= 0) {
                    this.message = '每日售卖数量必须大于0，请检查！';
                    this.open();
                    loading.close();
                    return;
            ...
            if (this.oCommissionWay == 1 || this.oCommissionWay == 2) {
                if (
                    this.oFirstCommission == '' ||
                    this.oFirstCommission <= 0 ||
                    this.oSecondCommission == '' ||
                    this.oSecondCommission <= 0
                ) {
                    this.message = '分销提成金额必须大于0！';
                    this.open();
                    loading.close();
            ...
            setTimeout(() => {
                ...
                jsonArr.push({ key: 'verifyDate', value: this.form.verifyDate });
                jsonArr.push({ key: 'endVerifyDate', value: this.form.endVerifyDate });
                jsonArr.push({ key: 'baseSaleNumber', value: this.oBaseSaleNumber });
                jsonArr.push({ key: 'dayLimitStatus', value: this.form.dayLimitStatus });
                if (this.form.dayLimitStatus == 2) {
                    jsonArr.push({ key: 'dayLimitNumber', value: this.form.dayLimitNumber });
                }
                if (this.form.commodityType == 4) {
                    let partnerCode = [];
                    for (let i = 0; i < this.selectedPartner.length; i++) {
                ...
                if (this.form.commodityType == 1) {
                    ...
                    if (this.couponId && this.couponId != '') {
                        ...
                        if (this.oEffectiveType == 1) {
                            ...
                            jsonArr.push({ key: 'couponExpireDay', value: this.form.couponExpireDay });
                        } else {
                ...
                } else if (this.form.commodityType != 1) {
                ...
                if (this.form.changeType != 1) {
                    jsonArr.push({ key: 'commissionWay', value: this.oCommissionWay });
                    if (this.oCommissionWay == 1 || this.oCommissionWay == 2) {
                        jsonArr.push({ key: 'firstCommission', value: this.oFirstCommission });
                        jsonArr.push({ key: 'secondCommission', value: this.oSecondCommission });
                    }
                }
                let sign = md5(preSign(jsonArr));
                jsonArr.push({ key: 'sign', value: sign });
        ...
        copyActivity(index, row) {
            ...
            let params = ParamsAppend(jsonArr);
            https
                .fetchPost('/goldCommodity/getCopyGoldCommodityById', params)
                .then(data => {
                    loading.close();
                    if (data.data.code == 'success') {
                        this.dialogFormVisible = true;
                        ...
                        if (JSON.parse(Decrypt(data.data.data)).goldCommodity.pickupType == 1) {
                            this.oForm.pickupType = '1';
                        }
                        if (JSON.parse(Decrypt(data.data.data)).goldCommodity.pickupType == 2) {
                            this.oForm.pickupType = '2';
                        }
                        this.changeSkuList = JSON.parse(Decrypt(data.data.data)).commoditySku;
                        if (JSON.parse(Decrypt(data.data.data)).goldCommodity.isUseSku == 1) {
                            this.oForm.isUseSku = 1;
                        }
                        if (JSON.parse(Decrypt(data.data.data)).goldCommodity.isUseSku == 2) {
                            this.oForm.isUseSku = 2;
                        }
                        this.oForm.image_url = JSON.parse(Decrypt(data.data.data)).goldCommodity.imageUrl;
                        this.oForm.memo = JSON.parse(Decrypt(data.data.data)).goldCommodity.memo;
                        this.oForm.store = '';
                        this.oForm.expireDay = JSON.parse(Decrypt(data.data.data)).goldCommodity.expireDay;
                        this.oForm.originalPrice = JSON.parse(Decrypt(data.data.data)).goldCommodity.originalPrice;
                        this.oForm.details = JSON.parse(Decrypt(data.data.data)).goldCommodity.details;
                        // this.form.alreadyChangedNumber = JSON.parse(Decrypt(data.data.data)).goldCommodity.alreadyChangedNumber;
                        this.oForm.gold = JSON.parse(Decrypt(data.data.data)).goldCommodity.gold;
                        this.oForm.money = JSON.parse(Decrypt(data.data.data)).goldCommodity.money;
                        this.oForm.cinemaCodes = JSON.parse(Decrypt(data.data.data)).goldCommodity.cinemaCodes;
                        this.oCheckedCities = this.oForm.cinemaCodes.split(',');
                        this.oForm.commodityType = JSON.parse(Decrypt(data.data.data)).goldCommodity.commodityType;
                        if (JSON.parse(Decrypt(data.data.data)).goldCommodity.commodityType == 1) {
                            this.ticketIds = JSON.parse(Decrypt(data.data.data)).goldCommodity.ticketIds;
                            this.couponId = JSON.parse(Decrypt(data.data.data)).goldCommodity.ticketIds;
                            this.groupName = JSON.parse(Decrypt(data.data.data)).goldCommodity.couponName;
                        ...
                        this.oForm.assign_info = JSON.parse(Decrypt(data.data.data)).goldCommodity.assignInfo;
                        this.oForm.limitNumber = JSON.parse(Decrypt(data.data.data)).goldCommodity.limitNumber;
                        this.oForm.sort = JSON.parse(Decrypt(data.data.data)).goldCommodity.sort;
                        this.oForm.endDate = JSON.parse(Decrypt(data.data.data)).goldCommodity.endDate;
                        this.oForm.verifyDate = JSON.parse(Decrypt(data.data.data)).goldCommodity.verifyDate;
                        this.oForm.endVerifyDate = JSON.parse(Decrypt(data.data.data)).goldCommodity.endVerifyDate;
                        this.oForm.couponExpireDay = JSON.parse(Decrypt(data.data.data)).goldCommodity.couponExpireDay;
                        this.cities = JSON.parse(Decrypt(data.data.data)).cinemas;
                        this.oForm.laterDays = JSON.parse(Decrypt(data.data.data)).goldCommodity.laterDays;
                        this.oForm.startEffectiveDate = JSON.parse(Decrypt(data.data.data)).goldCommodity.startEffectiveDate;
                        this.oForm.endEffectiveDate = JSON.parse(Decrypt(data.data.data)).goldCommodity.endEffectiveDate;
                        this.oForm.baseSaleNumber = JSON.parse(Decrypt(data.data.data)).goldCommodity.baseSaleNumber;
                        //商品类型下拉选显示对应的选项
                        for (let x in this.commodityType) {
                            if (this.commodityType[x].value == JSON.parse(Decrypt(data.data.data)).goldCommodity.commodityType) {
                                this.oForm.commodityType = this.commodityType[x].value;
                                break;
                            }
                        }
                        //商品类别下拉选显示对应的选项
                        for (let x in this.commodityCategoryType) {
                            if (
                                this.commodityCategoryType[x].value == JSON.parse(Decrypt(data.data.data)).goldCommodity.commodityCategory
                            ) {
                                this.oForm.commodityCategory = this.commodityCategoryType[x].value;
                                break;
                            }
                        }
                        //是否今日大牌下拉选显示对应的选项
                        for (let x in this.topStatusList) {
                            if (this.topStatusList[x].value == JSON.parse(Decrypt(data.data.data)).goldCommodity.topStatus) {
                                this.oForm.topStatus = this.topStatusList[x].value;
                                break;
                            }
                        }
                        //是否推荐下拉选显示对应的选项
                        for (let x in this.topStatusList) {
                            if (this.topStatusList[x].value == JSON.parse(Decrypt(data.data.data)).goldCommodity.recommendStatus) {
                                this.oForm.recommendStatus = this.topStatusList[x].value;
                                break;
                            }
                        }
                        //兑换方式下拉选显示对应的选项
                        for (let x in this.showType) {
                            if (this.showType[x].value == JSON.parse(Decrypt(data.data.data)).goldCommodity.changeType) {
                                this.oForm.change_type = this.showType[x].value;
                                break;
                            }
                        }
                        ...
                        for (let x in this.effectiveType) {
                            if (this.effectiveType[x].value == JSON.parse(Decrypt(data.data.data)).goldCommodity.effectiveType) {
                                this.oForm.effectiveType = this.effectiveType[x].value;
                                break;
                            }
                        }
                        // console.log(this.oEffectiveType);
                        ...
                        for (let x in this.showStatus) {
                            if (this.showStatus[x].value == JSON.parse(Decrypt(data.data.data)).goldCommodity.status) {
                                this.oForm.status = this.showStatus[x].value;
                                break;
                            }
                        }
                        ...
                        for (let x in this.assignType) {
                            if (this.assignType[x].value == JSON.parse(Decrypt(data.data.data)).goldCommodity.assignType) {
                                this.oForm.assign_type = this.assignType[x].value;
                                break;
                            }
                        }
                        ...
                        for (let x in this.limitType) {
                            if (this.limitType[x].value == JSON.parse(Decrypt(data.data.data)).goldCommodity.limitType) {
                                this.oForm.limit_type = this.limitType[x].value;
                                break;
                            }
                        }
                        for (let x in this.dayLimitStatus) {
                            if (this.dayLimitStatus[x].value == JSON.parse(Decrypt(data.data.data)).goldCommodity.dayLimitStatus) {
                                this.oForm.dayLimitStatus = this.dayLimitStatus[x].value;
                                break;
                            }
                        }
                        if (this.oForm.dayLimitStatus == 2) {
                            this.oForm.dayLimitNumber = JSON.parse(Decrypt(data.data.data)).goldCommodity.dayLimitNumber;
                        }
                        //是否快递下拉选显示对应的选项
                        for (let x in this.supportType) {
                            if (this.supportType[x].value == JSON.parse(Decrypt(data.data.data)).goldCommodity.supportExpressStatus) {
                                this.oForm.supportExpressStatus = this.supportType[x].value;
                                break;
                            }
                        }
                    } else if (data.data.code == 'nologin') {
        ...
        getMenu() {
            ...
            setTimeout(() => {
                ...
                let name = this.query.name;
                let changeSort = this.query.changeSort;
                if (!cinemaCodes) {
                    cinemaCodes = '';
                }
                if (!changeSort) {
                ...
                jsonArr.push({ key: 'changeSort', value: changeSort });

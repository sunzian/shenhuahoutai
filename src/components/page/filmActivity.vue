            ...
            >
                ...
                <el-table-column prop="name" label="适用制式" width="100">
                    <template slot-scope="scope">
                        ...
                        >{{scope.row.filmFormatName}}
                        </el-tag>
                        <el-tag
                                v-else-if="scope.row.selectFilmFormatType == 2"
                        >除{{scope.row.filmFormatName}}外所有制式
                        </el-tag>
                    </template>
                </el-table-column>
                <el-table-column prop="name" label="适用影片">
                    <template slot-scope="scope">
                        ...
                        <el-tag
                                v-else-if="scope.row.selectFilmType == 2"
                        >除{{scope.row.filmName}}外所有影片
                        </el-tag>
                    </template>
                </el-table-column>
                <el-table-column prop="sort" label="限购总数" width="90">
                    <template slot-scope="scope">
                        ...
                        <el-tag
                                v-else-if="scope.row.isLimitTotal == 1"
                        >{{scope.row.totalNumber}}/{{scope.row.totalSurplus}}
                        </el-tag>
                    </template>
                </el-table-column>
        ...
        <el-dialog :close-on-click-modal="false" title="新增"  :visible.sync="dialogFormVisible">
            <el-form :model="oForm">
                ...
                <el-form-item :required="true" label="选择影厅" :label-width="formLabelWidth">
                    <el-radio-group v-model="oForm.selectHallType" @change="clearScreenCode()" :disabled='hallType'>
                ...
                <el-form-item :required="true" label="选择制式" :label-width="formLabelWidth">
                    ...
                    <el-checkbox-group
                            v-model="oForm.formatCode"
                            @change="selectFormat"
                            v-if="oForm.selectMovieType != 0"
                    >
                        <el-checkbox
                ...
                <el-form-item :required="true" label="立减金额" :label-width="formLabelWidth" v-if="oForm.reduceType == 2">
                    减
                    <el-input placeholder="(每张票价-立减金额)*张数" style="width: 250px" v-model="oForm.discountMoney"
                              autocomplete="off"></el-input>
                </el-form-item>
                ...
                <el-form-item label="适用场次时间" :label-width="formLabelWidth">
                    <el-time-picker
                            is-range
                            type="date"
                            format="HH:mm:ss"
                ...
                <el-form-item :required="true" label="活动总张数" :label-width="formLabelWidth">
                ...
                <el-form-item
                        :required="true"
                        label="限制影票张数"
                        v-if="oForm.oCanNum==1"
                        :label-width="formLabelWidth"
                >
                    <el-input style="width: 150px" v-model="oForm.oNum" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item :required="true" label="限制个人张数" :label-width="formLabelWidth">
                ...
                >
                    ...
                    <div>限购时间单位为'日'，单位内个人总张数输入'2'，即每日限制使用该活动购买2张影票</div>
                </el-form-item>
...
4、和券是否共用 限制购票张数等等"
        ...
        <el-dialog :close-on-click-modal="false" title="修改" :visible.sync="editVisible">
            <el-form ref="form" :model="form">
                ...
                <el-form-item :required="true" label="选择影厅" :label-width="formLabelWidth">
                    <el-radio-group v-model="oSelectHallType" @change="clearScreenCode()" :disabled="hallType">
                        <el-radio label="0">全部影厅</el-radio>
                        <el-radio label="1">指定影厅参加</el-radio>
                        <el-radio label="2">指定影厅不参加</el-radio>
                    </el-radio-group>
                ...
                <el-form-item :required="true" label="立减金额" :label-width="formLabelWidth" v-if="oReduceType == 2">
                    减
                    <el-input placeholder="(每张票价-立减金额)*张数" style="width: 250px" v-model="oDiscountMoney"
                              autocomplete="off"></el-input>
                </el-form-item>
    ...
    export default {
        ...
        methods: {
            ...
            addPage() {
                ...
                https
                    .fetchPost('/filmDiscountActivity/addPage', '')
                    .then(data => {
                        loading.close();
            ...
            addRole() {
                ...
                if (!this.oForm.oCanNum) {
                    ...
                    if (!this.oForm.oNum) {
                        this.message = '限制影票张数不能为空，请检查！';
                        this.open();
                        loading.close();
                        return;
                    }
                    if (this.oForm.oNum <= 0) {
                        this.message = '限制影票张数必须大于0';
                        this.open();
                        loading.close();
                        return;
                    }
                ...
                if (this.dialogFormVisible == true) {
                    https
                        .fetchPost('/filmDiscountActivity/addActivity', params)
                        .then(data => {
                            //新增
                            loading.close();
                            console.log(data);
            ...
            delChange(index, row) {
                ...
                this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                    ...
                    .then(() => {
                        ...
                        let params = ParamsAppend(jsonArr);
                        https
                            .fetchPost('/filmDiscountActivity/deleteById', params)
                            .then(data => {
                                if (data.data.code == 'success') {
            ...
            addChange(index, row) {
                ...
                let params = ParamsAppend(jsonArr);
                https
                    .fetchPost('/filmDiscountActivity/getTimesById', params)
                    .then(data => {
                        //查询可用时间段
                        loading.close();
                        console.log(data);
                ...
                https
                    .fetchPost('/filmDiscountActivity/getActivityById', params)
                    .then(data => {
                        loading.close();
                        console.log(JSON.parse(Decrypt(data.data.data)));
                        if (data.data.code == 'success') {
                            ...
                            this.screenInfo = [];
                            if (
                                JSON.parse(Decrypt(data.data.data)).discountActivity.filmCode &&
                                JSON.parse(Decrypt(data.data.data)).discountActivity.filmName
                            ) {
                            ...
                            for (let x in this.canUse2) {
                                if (this.canUse2[x].value == JSON.parse(Decrypt(data.data.data)).discountActivity.isHolidayValid) {
                            ...
                            for (let x in this.canUse1) {
                                if (this.canUse1[x].value == JSON.parse(Decrypt(data.data.data)).discountActivity.isLimitTotal) {
                                    this.oIsLimitTotal = this.canUse1[x].value;
                                    break;
                                }
                            }
                            for (let x in this.canUse1) {
                                if (this.canUse1[x].value == JSON.parse(Decrypt(data.data.data)).discountActivity.isLimitSingle) {
            ...
            exChanger() {
                ...
                let params = ParamsAppend(jsonArr);
                https
                    .fetchPost('/filmDiscountActivity/updateActivityById', params)
                    .then(data => {
                        loading.close();
                        // console.log(JSON.parse(Decrypt(data.data.data)));
                        if (data.data.code == 'success') {
            ...
            changeStatus(index, row) {
                ...
                this.$confirm('是否确定' + this.rowMess + '此活动?', '提示', {
                    ...
                    .then(() => {
                        ...
                        let params = ParamsAppend(jsonArr);
                        https
                            .fetchPost('/filmDiscountActivity/updateStatusById', params)
                            .then(data => {
                                loading.close();
                                console.log(data);
                                // console.log(JSON.parse(Decrypt(data.data.data)));
            ...
            getMenu() {
                ...
                https
                    .fetchPost('/filmDiscountActivity/page', params)
                    .then(data => {
                        loading.close();
                        if (data.data.code == 'success') {

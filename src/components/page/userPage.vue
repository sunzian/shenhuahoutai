    ...
    <div>
        ...
        <div class="container">
            ...
            >
                ...
                <el-table-column prop="memo" label="描述" width="240">
                    <template slot-scope="scope">{{scope.row.memo}}</template>
                </el-table-column>
                <el-table-column prop="memo" label="联系方式" width="160">
                    <template slot-scope="scope">{{scope.row.callNumber}}</template>
                </el-table-column>
                <el-table-column prop="memo" label="影院公司名" width="160">
                    <template slot-scope="scope">{{scope.row.businessName}}</template>
                </el-table-column>
                <el-table-column prop="memo" label="登陆时间" width="190">
        ...
        <!--新增弹出框-->
        <el-dialog :close-on-click-modal="false" title="新增账号" :visible.sync="dialogFormVisible">
            <el-form v-model="oForm">
                ...
                <el-form-item :required="true" label="密码" :label-width="formLabelWidth">
                    <el-input type="password" style="width: 250px" v-model="oForm.userPass" autocomplete="off"></el-input>
                </el-form-item>
    ...
    export default {
        ...
        methods: {
            ...
            getCheckedKeys() {//新增数据操作
                ...
                jsonArr.push({
                    key: 'menuIds',
                    value: this.$refs.tree.getCheckedKeys().concat(this.$refs.tree.getHalfCheckedKeys())
                });
                let sign = md5(preSign(jsonArr));
                jsonArr.push({ key: 'sign', value: sign });
                let params = ParamsAppend(jsonArr);
            ...
            exChanger() {
                ...
                jsonArr.push({
                    key: 'menuIds',
                    value: this.$refs.tree_.getCheckedKeys().concat(this.$refs.tree_.getHalfCheckedKeys())
                });
                let sign = md5(preSign(jsonArr));
                jsonArr.push({ key: 'sign', value: sign });
                console.log(jsonArr);
